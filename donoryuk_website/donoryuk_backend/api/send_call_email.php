...
// BARIS DEBUGGING: Aktifkan tampilan error PHP di browser
...
if (
    ...
    try {
        // BARIS DEBUGGING: Aktifkan debug output PHPMailer
        ...
        $mail->isSMTP();
        $mail->Host       = 'mail.donoryuk.xyz';   // GANTI INI DENGAN HOST SMTP ANDA (misal: mail.donoryuk.xyz)
        $mail->SMTPAuth   = true;
        $mail->Username   = 'admin@donoryuk.xyz';  // GANTI INI DENGAN EMAIL PENGIRIM ANDA
        $mail->Password   = 'Ml8=}j]ZS,)14g,R';            // GANTI INI DENGAN KATA SANDI ASLI EMAIL PENGIRIM ANDA
        ...
        $mail->setFrom('admin@donoryuk.com', 'Admin Donor Yuk!');
        $mail->addAddress($recipient_email, $fullname);

        // Konten Email
        $mail->isHTML(true);
        $mail->Subject = 'Panggilan Donor Darah Anda - Donor Yuk!';
        // PERBAIKAN DI SINI: Menyisipkan link dengan kutip ganda dan tanpa spasi aneh.
        // Pastikan tidak ada spasi atau karakter tersembunyi setelah `href=` atau di dalam URL.
        $mail->Body    = "
            <!DOCTYPE html>
            <html>
            <head>
